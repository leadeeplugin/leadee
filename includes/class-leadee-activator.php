...
class leadee_Activator
{
    ...
    private function leadee_install_plugin()
    {
        ...
        $settings = array(
            array('leads-table-colums', 'source', 1),
            array('leads-table-colums', 'first_url_parameters', 1),
            array('leads-table-colums', 'device_browser', 1),
            array('leads-table-colums', 'device_screen_size', 1),
            array('leads-table-colums', 'dt', 1),
            array('setting-target', 'month-target', 10)
        );

        foreach ($settings as $setting) {
            $this->wpdb->query($this->wpdb->prepare("INSERT IGNORE INTO " . $this->wpdb->prefix . "leadee_settings (`setting_type`, `option`, `value`) VALUES (%s, %s, %d)", $setting));
        }
    ...
    {
        $allowed_types = ['advert', 'serp', 'social'];
